 ...
 *
 * 司会画面と投影画面の**両方**が使う。どちらか一方だけを開いていても、
 * 時間が来たら自動的に締め切られる（会場では投影だけを出していることも、
 * 手元の司会画面だけを見ていることもある）。
 *
 * 大事なところ:
...
export function useExpiryLock({
   ...
   * 表示上の残り時間が 0 になったか。
   *
   * **締切時刻が分かっている状態で 0 になったときだけ true にすること。**
   * 締切時刻が未取得のうちに true を渡すと、本来より早く 1 度だけ呼んでしまい、
   * 二重実行防止に阻まれて本当の締切時刻に何も起きなくなる。
   */
  expired: boolean;
