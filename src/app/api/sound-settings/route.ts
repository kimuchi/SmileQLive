/**
 * GET  /api/sound-settings  いま鳴る効果音の一覧
 * POST /api/sound-settings  1 音を差し替える（multipart/form-data）
 *
 * フィールド:
 ...
 * 「音を差し替えたいだけ」でログインを求めると、当日その場で直せない。
 * その代わり **URL を知っている人は誰でも差し替えられる**ので、
 * 外へ公開する場合は Cloud Run 側で入口を絞ること（docs/SOUNDS.md）。
 *
 * ログインの代わりに置いている歯止め:
 *   - 同一オリジンからの送信だけ受ける
 *   - 大きさの上限（宣言サイズと実データの両方で見る）
 *   - 形式は拡張子でも Content-Type でもなく実データで判定する
 *   - 回線ごとの回数制限
 *
 ...
 * 差し替えの回数制限。
 *
 * 12 音を入れ直しても 12 回。会場でやり直しても 30 回あれば足りる。
 * ログインを求めない経路なので、置かないと保存先をいくらでも使わせられる。
 */
const UPLOAD_RATE_LIMIT = { limit: 30, windowMs: 60_000 } as const;

export const GET = withRoute<SoundSettingsResponse>('sound_settings.list', async () => {
  return jsonOk<SoundSettingsResponse>(await listSoundSettings());
});

export const POST = withRoute<SoundSettingsResponse>(
  'sound_settings.upload',
  async ({ request }) => {
    ...
    checkRateLimit(clientKeyFromRequest(request, 'sound_settings.upload'), UPLOAD_RATE_LIMIT);
