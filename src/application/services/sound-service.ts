...
import { getRoom } from '@/infrastructure/firebase/repositories/room-repository';
...
export async function listSoundSettings(): Promise<SoundSettingsResponse> {
  ...
  const settings = await getSoundSettings(user.uid);
...
export async function uploadSound(input: UploadSoundInput): Promise<SoundSettingsResponse> {
  ...
  const objectPath = buildSoundObjectPath(user.uid, assetId, soundExtensionFor(detected.mime));
  const bucket = mediaBucketName();

  await uploadSoundObject({ objectPath, buffer: bytes, contentType: detected.mime });

  const { settings, replaced } = await putSoundOverride(user.uid, input.name, {
    assetId,
    bucket,
    objectPath,
...
export async function resetSound(name: SoundName): Promise<SoundSettingsResponse> {
  const { user } = await requireHostUser();
  const { settings, removed } = await removeSoundOverride(user.uid, name);
 ...
 */
export async function buildSoundManifest(roomId: string): Promise<Record<SoundName, string>> {
  const room = await getRoom(roomId);
  const settings = room ? await getSoundSettings(room.ownerId) : null;
