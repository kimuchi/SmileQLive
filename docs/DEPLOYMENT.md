...
> **バケットの用意そのものは自動です。** `npm run gcp:bootstrap -- <env>` が
> 設定ファイルの `mediaBucket` の名前でバケットを作り、実行サービスアカウントへ
> 「Storage オブジェクト管理者」を付けます。`npm run deploy` がその名前を
> 環境変数 `MEDIA_BUCKET` として Cloud Run へ渡します。手で設定する場所はありません。
