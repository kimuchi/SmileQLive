...
クイズは**所有者ごと**に分かれています。既定では、ほかの司会者の一覧には出ません。

所有者は、クイズ編集画面の「ほかの司会者と共有」から共有相手を追加できます。
...
- 共有相手は**司会者として登録済み**（`profiles/{uid}` がある）でなければなりません。
  未登録のアドレスへは共有できません（サーバー側で拒否します）。
- 所有者は 1 人のままです。同じクイズを複数人が同時に編集して壊すことを避けるためです。
- ルームの所有者は**作成した本人**になります。共有元の司会者が他人のルームを操作することはありません。
- 共有しても**参加者から見えるものは変わりません**。Security Rules は所有者と共有相手だけに
  読み取りを許可し、参加者・投影担当は引き続き到達できません。

編集も必要な場合は、共有ではなく「複製」を使ってください（複製の所有者は複製した本人になり、
共有設定は引き継ぎません）。
