...
### 3.4 1 つのドキュメントへの集中を避ける

Firestore は **1 つのドキュメントへの書き込みが毎秒 1 回程度**を想定しています。
会場では「全員が同時に同じことをする」ため、同じドキュメントへ人数ぶんの
読み書きが集中しやすく、ここが壊れると人数に比例せず**二乗で**悪化します。

とくに、**同じドキュメントを読んで書き戻すトランザクション**を
同時に走らせてはいけません。片方が必ずやり直しになるため、
人数が増えるほど失敗が増えます。

#### 参加登録（開場直後）

参加登録は 1 つのトランザクションで `rooms/{id}` を読み、
`participantCount + 1` を書き戻していました。
500 人で試したところ、**499 人が ABORTED で入れませんでした**
（`tests/load/scale.test.ts`）。

現在は次のようにしています。

- 二重登録を防いでいるのは、参加者 uid をドキュメント ID にした `create()`。
  これは UNIQUE 制約と同じ強さで、トランザクションは要らない
- 人数はドキュメント単位の加算（`FieldValue.increment`）で足す。読み書きの
  往復が無いので競合しない
- 全員が購読している `public/state` への反映だけは 400ms に 1 回まで間引く
- 代償として定員判定はわずかに緩くなる（同時に来た人が数人ぶん超えることがある）

#### 回答進捗

500 人が 10 秒で回答すると、進捗ドキュメントへの書き込みが毎秒 50 回になります。

- 回答ごとの `answeredCount` 更新は **400ms のスロットリング**で間引く
- 間引きの判定は `count()` 集計より**先に**行う。あとで判定すると、
  捨てるだけの集計を人数ぶん走らせてしまう
...
順位を出すには参加者全員の得点を読む必要があります。
正解発表では全員が同時に取りに来るため、そのままでは
「人数 × 人数」の読み取りになります（500 人なら 1 回の発表で 25 万件）。

得点が動くのは回答受付中だけで、フェーズが変われば `stateVersion` が増えます。
そこで **(ルーム, stateVersion)** をキーにして、インスタンス内で一度だけ作って
使い回します（`src/application/services/ranking-cache.ts`）。
覚えているのは導出できる結果だけで、進行状態そのものは持ちません。

#### 在席時刻

`lastSeenAt` は Snapshot のたびに書いていたため、全員の画面が更新される瞬間に
人数ぶんの書き込みが発生していました。しかもこの値は複合インデックスの
並び替えキーで、同じ向きに増える値を一斉に書くとインデックス側が詰まります。
30 秒以内に更新済みなら書き直しません。

#### 確かめかた

```bash
npm run load:test                          # 500 人
