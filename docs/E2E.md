...
`tests/e2e/poll-flow.spec.ts` は投票モードを通しで実行します（同じ環境変数で動きます）。

投票の通しテストで固めているのは次の 5 つです。

1. **1 台につき 1 票**。同じ端末から二度送っても増えない。
2. **投票中に票の内訳が参加者へ渡らない**（応答に載っていないことまで見る）。
3. 締め切った時点で集計が凍り、司会が票数を直せる。点数と順位は数え直される。
4. **発表した順位までしか届かない**（まだ出していない順位の名前が応答に無い）。
5. 発表を始めたら受付へ戻せない。
