...
--
-- 既知の制限:
--   本ファイルは 1 つの DO ブロック＝1 トランザクションで動くため、
--   now() が固定され elapsed_ms は常に 0 になる。
--   したがって「同点時に所要時間が短い方を上位にする」規則はここでは検証できない
--   （その規則は tests/unit の compareForRanking テストで検証している）。
--   本番では RPC 1 回＝1 トランザクションなので elapsed_ms は正しく計測される。
-- =============================================================================
